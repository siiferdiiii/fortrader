...
const Methods = {
    init() {
        ...
        this.bindEvents();
        this.render();
        // Realtime: auto-refresh saat data berubah di DB
        Storage.subscribeToMethods(() => this.render());
    },
    ...
    async deleteMethod(id) {
        if (!confirm('Yakin ingin menghapus metode ini?')) return;
        await Storage.deleteMethod(id);
        await this.render();
    ...
    async render() {
      ...
      `;
            card.querySelector('.btn-edit-method').addEventListener('click',   () => this.openModal(method));
            card.querySelector('.btn-delete-method').addEventListener('click', () => this.deleteMethod(method.id));
